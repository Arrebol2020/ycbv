...
class Trainer:
    ...
    def train(self, batch_size, epoches, lr, max_size):
        ...
        for inc_i in range(dataset.batch_num):
            ...
            train_xs, train_ys = exemplar.get_exemplar_train()  # 获取旧任务中的样本
            train_xs.extend(train_x)
            train_ys.extend(train_y)

            train_xs.extend(val_x)  # 这里为什么将验证集也加入到训练集中训练
            ...
            for epoch in range(epoches):
                ...
                if inc_i > 0:
                    self.stage1_distill(train_data, criterion, optimizer)  # 训练第一个之后的模型
                else:
                    self.stage1(train_data, criterion, optimizer)  # 训练第一个模型
                scheduler.step()
            ...
            if inc_i > 0:
                for epoch in range(epoches):
                    ...
                    if epoch % 50 == 0:
                        acc = self.test(test_data)
                        test_acc.append(acc)
            for i, layer in enumerate(self.bias_layers):  # 查看bias correction layer的参数
                layer.printParam(i)

            self.previous_model = deepcopy(self.model)  # 复制模型
            acc = self.test(test_data)
            test_acc.append(acc)
    ...
    def stage1_distill(self, train_data, criterion, optimizer):
        ...
        ce_losses = []
        T = 2
        alpha = (self.seen_cls - 20) / self.seen_cls  # Q2:可能存在问题，不知道这里是否是原文中的λ
        ...
        for i, (image, label) in enumerate(tqdm(train_data)):
            ...
            loss_hard_target = nn.CrossEntropyLoss()(p[:, :self.seen_cls], label)  # 交叉熵损失
            loss = loss_soft_target * T * T + (1-alpha) * loss_hard_target  # Q3:这里可能存在问题，蒸馏损失前的系数可能与原文本不一致
            optimizer.zero_grad()
            loss.backward(retain_graph=True)
            optimizer.step()
